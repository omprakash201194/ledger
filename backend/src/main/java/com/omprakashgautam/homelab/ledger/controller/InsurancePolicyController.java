...
public class InsurancePolicyController {
    ...
    @GetMapping
    public ResponseEntity<List<InsurancePolicy>> list(@AuthenticationPrincipal UserDetailsImpl principal) {
        return ResponseEntity.ok(insurancePolicyService.findAll(principal.getUsername()));
    }

    @GetMapping("/{id}")
    public ResponseEntity<InsurancePolicy> get(@PathVariable UUID id, @AuthenticationPrincipal UserDetailsImpl principal) {
        return ResponseEntity.ok(insurancePolicyService.findById(id, principal.getUsername()));
    }

    @PostMapping
    public ResponseEntity<InsurancePolicy> create(@Valid @RequestBody InsurancePolicyRequest request,
                                            @AuthenticationPrincipal UserDetailsImpl principal) {
        return ResponseEntity.ok(insurancePolicyService.create(request, principal.getUsername()));
    }

    @PutMapping("/{id}")
    public ResponseEntity<InsurancePolicy> update(@PathVariable UUID id, @Valid @RequestBody InsurancePolicyRequest request,
                                             @AuthenticationPrincipal UserDetailsImpl principal) {

...
public class TrustedPersonController {
    ...
    @GetMapping
    public ResponseEntity<List<TrustedPerson>> list(@AuthenticationPrincipal UserDetailsImpl principal) {
        return ResponseEntity.ok(trustedPersonService.findAll(principal.getUsername()));
    }

    @GetMapping("/{id}")
    public ResponseEntity<TrustedPerson> get(@PathVariable UUID id, @AuthenticationPrincipal UserDetailsImpl principal) {
        return ResponseEntity.ok(trustedPersonService.findById(id, principal.getUsername()));
    }

    @PostMapping
    public ResponseEntity<TrustedPerson> create(@Valid @RequestBody TrustedPersonRequest request,
                                            @AuthenticationPrincipal UserDetailsImpl principal) {
        return ResponseEntity.ok(trustedPersonService.create(request, principal.getUsername()));
    }

    @PutMapping("/{id}")
    public ResponseEntity<TrustedPerson> update(@PathVariable UUID id, @Valid @RequestBody TrustedPersonRequest request,
                                             @AuthenticationPrincipal UserDetailsImpl principal) {

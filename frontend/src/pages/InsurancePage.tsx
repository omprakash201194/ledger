  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-800 dark:text-gray-200">Insurance</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">{policies.length} polic{policies.length !== 1 ? 'ies' : 'y'}</p>
        </div>
      ...
      <SectionIntro sectionKey="insurance" note="Life, health, accident and general insurance policies. Insurance is usually the first thing a family must claim — they cannot claim what they do not know exists. Include the policy document location; finding the original is often the hardest part." />
            ...
            <div key={p.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-medium text-gray-800 dark:text-gray-200">{p.insurer}</span>
                  <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 dark:text-gray-500 px-2 py-0.5 rounded-full">{p.policyType.replace(/_/g, ' ')}</span>
                </div>
                <div className="flex gap-4 mt-1 flex-wrap">
                  {p.policyNumber && <span className="text-xs text-gray-400 dark:text-gray-500">#{p.policyNumber}</span>}
                  {p.lifeAssured && <span className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">Life assured: {p.lifeAssured}</span>}
                  {p.sumAssured && <span className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">Sum: {fmt(p.sumAssured)}</span>}
                  {p.premiumAmount && <span className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">Premium: {fmt(p.premiumAmount)}</span>}
                ...
                {p.maturityDate && <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Matures: {p.maturityDate}</p>}
      ...
      {deleteId && (
        <Modal title="Delete policy?" onClose={() => setDeleteId(null)}>
          <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500 mb-4">This will permanently remove the insurance policy record.</p>
          <div className="flex gap-3">

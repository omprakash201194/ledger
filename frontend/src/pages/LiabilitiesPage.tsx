  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Liabilities</h1>
          <p className="text-sm text-gray-500 mt-0.5">Outstanding: <span className="font-semibold text-red-600">{fmt(total)}</span></p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {liabilities.map(l => (
            <div key={l.id} className="bg-white rounded-xl border border-gray-200 px-4 py-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-medium text-gray-800">{l.lender}</span>
                  <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{l.liabilityType.replace(/_/g, ' ')}</span>
                </div>
                <div className="flex gap-4 mt-1 flex-wrap">
                  {l.accountNumber && <span className="text-xs text-gray-400">Acct: {l.accountNumber}</span>}
                  {l.emiAmount && <span className="text-xs text-gray-500">EMI: {fmt(l.emiAmount)}/mo</span>}
                  {l.tenureEndDate && <span className="text-xs text-gray-400">Ends: {l.tenureEndDate}</span>}
      ...
      {deleteId && (
        <Modal title="Delete liability?" onClose={() => setDeleteId(null)}>
          <p className="text-sm text-gray-600 mb-4">This will permanently remove the liability record.</p>
          <div className="flex gap-3">

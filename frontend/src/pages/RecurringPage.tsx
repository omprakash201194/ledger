...
export default function RecurringPage() {
  ...
  const save = async () => {
    setSaving(true)
    try {
      if (editing) await updateRecurringObligation(editing.id, form)
      else await createRecurringObligation(form)
      setShowModal(false)
      load()
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">No recurring obligations. Add EMIs, SIPs, or subscriptions.</p>
        </div>
      ) : (
        ...
        <Modal title={editing ? 'Edit Obligation' : 'Add Obligation'} onClose={() => setShowModal(false)}>
          <div className="space-y-4">
            ...
            <FormField label="Payee" required>
              <input className={inputCls} value={form.payee} onChange={e => setForm(f => ({ ...f, payee: e.target.value }))} placeholder="e.g. HDFC Bank, Netflix" />
            </FormField>
            <FormField label="Amount (₹)" required>
              <input className={inputCls} type="number" min="0" value={form.amount || ''} onChange={e => setForm(f => ({ ...f, amount: Number(e.target.value) }))} placeholder="0" />
            </FormField>

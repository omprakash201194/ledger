  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-800 dark:text-gray-200">Recurring Obligations</h1>
      ...
      <SectionIntro sectionKey="recurring" note="Auto-debits, SIPs, subscriptions and standing instructions — anything that keeps charging your account whether or not you act on it. Mark each one as Continue / Cancel / Transfer so your family knows what to stop and what to keep running." />
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {obligations.map(o => (
            <div key={o.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-3 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="font-medium text-gray-800 dark:text-gray-200">{o.payee}</span>
                  <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 dark:text-gray-500 px-2 py-0.5 rounded-full">{o.obligationType.replace(/_/g, ' ')}</span>
                  {o.actionOnDeath && (
                ...
                <div className="flex gap-4 mt-1 flex-wrap">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">{o.frequency.replace(/_/g, ' ')}</span>
                  {o.dueDay && <span className="text-xs text-gray-400 dark:text-gray-500">Due: {o.dueDay}th</span>}
                  {o.paymentSource && <span className="text-xs text-gray-400 dark:text-gray-500">via {o.paymentSource}</span>}
                </div>
      ...
      {deleteId && (
        <Modal title="Delete obligation?" onClose={() => setDeleteId(null)}>
          <p className="text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500 mb-4">This will permanently remove the recurring obligation record.</p>
